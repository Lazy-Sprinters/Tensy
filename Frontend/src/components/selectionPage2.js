...
export class selectionPage1 extends Component {
  ...
  curr = (x) => {
    ...
    Axios.post("http://localhost:8080/helper/check",userInfo)      .then((res) => {
  ...
  fn1 = (x) => {
    Axios.post("http://localhost:8080/manufacturer/acceptforconsideration", x)
      .then((res) => {
        console.log(res);
        // this.handleSearch(res);
  ...
  fn2 = (Man_id, Bid_id, Price_Per_Unit, Mode_Of_Delivery) => {
    ...
    Axios.post("http://localhost:8080/manufacturer/submitnego", x)
      .then((res) => {
        console.log(res);
        // this.handleSearch(res);
        window.location.reload();
      ...
      <>
        ...
        <TnCModal2
          ...
          onAgree={(Price_Per_Unit) =>
            this.fn2(y.Man_id, y.Bid_id, Price_Per_Unit, Mode_Of_Delivery)
          }
        />
        <TnCModal
          btnshow={true}
        ...
        />
        <Pre1 />
        <LoginNavbar userInfo={this.props.userInfo} /* tochange */ />
        {auth1 && (
          <StickyHeadTable2
            arr={currbids}
