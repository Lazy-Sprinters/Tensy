...
export class CenterProfileView extends React.Component {
  ...
  handleOtp = (centerInfo, id, value, flag) => {
    if (flag == 0) {
      ...
      Axios.post("http://localhost:8080/center/sendotp", data)
        .then((res) => {
  ...
  verifyOtp = (centerInfo, otp, flag, id, value) => {
    if (flag == 0) {
      ...
      Axios.post("http://localhost:8080/center/verifyotponupd", data)
        .then((res) => {
  ...
  EditDetails = (centerInfo, data) => {
    ...
    Axios.post("http://localhost:8080/center/update", ret)
      .then((res) => {
  ...
  handleShow = (PseudoFacilities, len) => {
    ...
    if (PseudoFacilities.length > 0) {
      code.push(
        <Table responsive="lg" size="sm" striped bordered hover>
          <thead>
            <tr>
              <th>#</th>
              <th>Facility</th>
              ...
              <tr>
                ...
                <td>
                  {i >= len && (
                    <DeleteOutlinedIcon
                      onClick={() =>
                        this.handleDelete(i, PseudoFacilities, len)
                      }
                    />
                  )}
                </td>
        ...
        <div style={{ marginLeft: "5vw" }} className="row">
          <Avatar
            ...
            src={
              ...
              centerInfo.data.center.FrontImage.toString()
            }
          />
          <div>
            <Typography
            ...
            <div className="row">
              ...
              >
                Opening Time: {centerInfo.data.center.OpeningTime}
              </Typography>
              <Typography
                style={{
                  marginTop: "-30px",
              ...
              >
                Closing Time: {centerInfo.data.center.ClosingTime}
              </Typography>
            </div>
          </div>
        </div>
                ...
                <>
                  <div style={{ marginLeft: "10px", marginTop: "25px" }}>
                    <Button
                      ...
                      disabled={
                        (!editProfile &&
                          Email == centerInfo.data.center.Email) ||
                        !complete
                      }
                      onClick={() =>
                        this.handleOtp(
                          centerInfo,
                          centerInfo.data.center._id,
                          Email,
                          0
                        )
                      }
                    >
                      {parseInt(Object.values({ ct })) == 0
                        ? "Send Otp"
                        : "Send Otp ( " +
                          parseInt(Object.values({ ct })) +
                          " sec )"}
                    </Button>
                  </div>
                  {sendEmailOtp && (
            ...
            <div className="row">
              <div className="txtfld1" style={{ marginLeft: "11vw" }}>
                <TextField
                  ...
                  type="number"
                  inputProps={{ min: 1000000000, max: 9999999999, step: 1 }}
                  margin="normal"
                  fullWidth
                />
              </div>
                ...
                <>
                  <div style={{ marginLeft: "10px", marginTop: "25px" }}>
                    <Button
                      ...
                      disabled={
                        (!editProfile &&
                          PhoneNo == centerInfo.data.center.PhoneNo) ||
                        !complete1
                      }
                      onClick={() =>
                        this.handleOtp(
                    ...
                    >
                      {parseInt(Object.values({ ct1 })) == 0
                        ? "Send Otp"
                        : "Send Otp ( " +
                          parseInt(Object.values({ ct1 })) +
                          " sec )"}
                    </Button>
                  </div>
                  {sendPhoneOtp && (
                  ...
                  <TextField
                    placeholder={
                      Password.length > 0
                        ...
                        : "Enter your password"
                    } /*change as the password changes to old or nothing*/
                    label={
                      Password.length > 0
                        ? "Enter your old password to edit"
                    ...
                    disabled={
                      !(
                        ...
                        (((centerInfo.data.center.Address != Address ||
                          ...
                          centerInfo.data.center.NearestLandmark !=
                            NearestLandmark ||
                          centerInfo.data.center.City != City ||
                          centerInfo.data.center.Pincode != Pincode ||
                          centerInfo.data.center.State != State ||
                          centerInfo.data.center.Country != Country ||
                          facilitiesList != facilities ||
                          Password.length >= 8) &&
                          (Email == centerInfo.data.center.Email ||
                            (Email == tempEmail &&
                              tempEmail != centerInfo.data.center.Email)) &&
                          (PhoneNo == centerInfo.data.center.PhoneNo ||
                            (PhoneNo == tempPhoneNo &&
                              tempPhoneNo !=
                                centerInfo.data.center.PhoneNo))) ||
                          ((Email == centerInfo.data.center.Email ||
                            (Email == tempEmail &&
                              tempEmail != centerInfo.data.center.Email)) &&
                            (PhoneNo == centerInfo.data.center.PhoneNo ||
                        ...
                        !(
                          ...
                          centerInfo.data.center.Country == Country &&
                          facilitiesList == facilities &&
                          centerInfo.data.center.Email == Email &&
                          PhoneNo == centerInfo.data.center.PhoneNo &&
                          Password.length < 8
                        )
                      )
