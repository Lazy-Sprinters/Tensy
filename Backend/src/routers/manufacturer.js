...
router.post("/manufacturer/login", async (req, res) => {
  try {
    const manufacturer = await Manufacturer.findbycredentials(
      req.body.Email,
      req.body.password
    );
...
router.post("/agreements/upd", async (req, res) => {
  try {
    // console.log(req.body);
    const allaggreements = await Agreement.find({
...
router.post("/manufacturer/agreements", async (req, res) => {
  try {
    ...
    res
      .status(200)
      .send({ Upcoming_Agreements, Live_Agreements, Completed_Agreements });
  } catch (err) {
    console.log(err);
    res.status(400).send();
  }
...
router.post("/manufacturer/openrfps", async (req, res) => {
  ...
  try {
    ...
    for (let i = 0; i < allrfps.length; i++) {
      ...
      showbuttons &=
        Helper.comparedatecurr(allrfps[i].DeadlineDate) == 0 ||
        Helper.comparedatecurr(allrfps[i].DeadlineDate) == 0;
...
router.post("/manufacturer/finalizedbids", async (req, res) => {
  try {
    const allfinalbids = await Bid.find({
    ...
    for (let i = 0; i < allfinalbids.length; i++) {
      if (allfinalbids[i].All_negotiation.length == 0) {
        ...
        const v = await Vendor.findOne({ _id: allfinalbids[i].vendor_id });
        const o =
          allfinalbids[i].All_negotiation[
            allfinalbids[i].All_negotiation.length - 1
          ];
        // console.log(o);
        ret.push({
          Bid_id: allfinalbids[i]._id,
...
router.post("/manufacturer/openbids", async (req, res) => {
  try {
    // console.log(req.body);
    const allopenbids = await Bid.find({
      rfp_id: req.body.Rfp_id,
      Status: false,
    });
    let ret = [];
    for (let i = 0; i < allopenbids.length; i++) {
      const selectedvendor = await Vendor.findOne({
        _id: allopenbids[i].vendor_id,
      });
      const currmanufacturer = await Manufacturer.findOne({
        _id: allopenbids[i].manufacturer_id,
      });
      const currentrfp = await Rpf.findOne({ _id: allopenbids[i].rfp_id });
      const o =
        allopenbids[i].All_negotiation[
          allopenbids[i].All_negotiation.length - 1
        ];
      ret.push({
        ...
        Mode_Of_Delivery: o.Quote_ModeofDelivery,
        Bidder:
          o.Quote_owner == allopenbids[i].vendor
            ? selectedvendor.CompanyName
            : currmanufacturer.CompanyName,
      });
    }
    // console.log(ret);
    res.status(200).send(ret);
...
router.post("/manufacturer/acceptbid", async (req, res) => {
  try {
    ...
    const currmanufacturer = await Manufacturer.findOne({
      _id: acceptedbid.manufacturer_id,
    });
    ...
    const newagreement = new Agreement({
      ...
      Unit: currentrfp.Unit,
      Cost_per_Unit:
        acceptedbid.All_negotiation.length == 0
          ? currentrfp.Cost_per_Unit
          : acceptedbid.All_negotiation[acceptedbid.All_negotiation.length - 1]
              .Quote_Cost_per_Unit,
