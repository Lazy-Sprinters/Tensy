...
router.post("/vendor/list", async (req, res) => {
  try {
    ...
    for (let i = 0; i < allrfps.length; i++) {
      const found = allrfps[i].Action_taken.find(
        (e) => e.vendor_id == req.body.id
      );
      if (found == undefined) s2.add(allrfps[i].Product_Name);
    }
...
router.post("/vendor/allvalidrfps", async (req, res) => {
  try {
    ...
    for (let i = 0; i < allsemivalidrfps.length; i++) {
      let found = allsemivalidrfps[i].Action_taken.find(
        (e) => e.vendor_id == req.body.Vendor_id
      );
      if (found == undefined) {
        const currmanu = await Manufacturer.findOne({
          _id: allsemivalidrfps[i].Manufacturer_id,
        });
        ret.push({
...
router.post("/vendor/allbids", async (req, res) => {
  try {
    ...
    for (let i = 0; i < allbids.length; i++) {
      ...
      if (
        allbids[i].Status == false ||
        allbids[i].All_negotiation.length == 0
      ) {
        ret.push({
        ...
        const obj =
          allbids[i].All_negotiation[allbids[i].All_negotiation.length - 1];
        ret.push({
          Bid_id: allbids[i]._id,
          Product: currentrfp.Product_Name,
          Rfp_id: currentrfp._id,
...
router.post("/vendor/negotiate1", async (req, res) => {
  try {
    ...
    const currman = await Manufacturer.findOne({
      _id: currbid.manufacturer_id,
    });
    ...
    const ret = {
      ...
      Mode_Of_Delivery: obj.Quote_ModeofDelivery,
      Bidder:
        obj.Quote_owner == currbid.vendor_id
          ? currven.CompanyName
          : currman.CompanyName,
    };
    res.status(200).send(ret);
